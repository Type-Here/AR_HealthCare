Shader "HoloMed/ButtonGray"
{
    Properties
    {
        ...
        _BaseColor      ("Base Color",      Color) = (0.17, 0.17, 0.17, 0.80)
        _TopColor       ("Top highlight",   Color) = (0.38, 0.38, 0.38, 0.85)
        _RimColor       ("Rim / edge glow", Color) = (0.75, 0.75, 0.75, 0.35)
        _RimPower       ("Rim sharpness",   Range(1,8))  = 3.0
        _BevelWidth     ("Bevel width",     Range(0,0.5))= 0.09
        _BevelBright    ("Bevel brightness",Range(0,2))  = 0.45
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float bevel    = (1.0 - edge) * _BevelBright * lerp(1.3, 0.4, bevelDir);
                col.rgb       += bevel;

                float rim = 1.0 - saturate(edge * _RimPower);
                col.rgb   = lerp(col.rgb, _RimColor.rgb, rim * _RimColor.a);

                col.rgb *= lerp(1.0, 0.72, _PressDepth);
